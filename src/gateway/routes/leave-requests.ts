...
// S6 (PRD-006b) owns `leave.ts` — the HR *configuration* surface mounted at
// /v1/people/leave and gated on `people`: leave types, policies, tenure bands,
// public holidays and the work week. This file is the employee-facing
// *consumption* surface mounted at /v1/leave and gated on `self`: preview,
// submit, cancel, my balances and the team calendar. Two routers, two mounts,
// two capability axes — they were built concurrently and both landed on the
// name `leave`, which is the only reason this one is `leave-requests.ts`.
...
export const leaveRequests = new Hono<AuthedEnv>();
 ...
 */
leaveRequests.get("/types", async (c) => {
 ...
 */
leaveRequests.get("/balances", async (c) => {
 ...
 */
leaveRequests.post("/preview", zValidator("json", previewSchema), async (c) => {
 ...
 */
leaveRequests.get("/requests", zValidator("query", listQuerySchema), async (c) => {
...
leaveRequests.post("/requests", zValidator("json", submitSchema), async (c) => {
  const body = c.req.valid("json");
 ...
 */
leaveRequests.get("/requests/:id", async (c) => {
  const tenantId = c.get("tenant").tenant_id;
 ...
 */
leaveRequests.post("/requests/:id/cancel", async (c) => {
  const tenantId = c.get("tenant").tenant_id;
 ...
 */
leaveRequests.get("/calendar", zValidator("query", calendarQuerySchema), async (c) => {
  const query = c.req.valid("query");

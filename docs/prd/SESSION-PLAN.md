...
| Roles | `src/auth/roles.ts` — exactly `admin`, `operator`, `finance`, `support`, `readonly`. PRD-000's role-based strategy (`admin` or `finance`) maps cleanly. **Do not add roles** without a decision. |

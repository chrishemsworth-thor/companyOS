...
|---|---|---|---|
| **Finance / Accounting** | `finance` — double-entry ledger, invoices, payments, daily `overdue-sweep` | ✅ `CollectionsAgent`, now **bounded and measured** ([docs](./modules/agents.md)) | **Strongest.** Append-only journal enforced in SQL, cron, and a working agent. PRD-002 shipped the guardrail layer (hours, holidays, kill switches, escalation gate), a 28-scenario eval suite and per-decision cost — so the agent is safe to leave running rather than needing supervision. |
| **Sales / Revenue** | `crm` (`source_module: sales`) — customers, deal pipeline, contact roles, customer health, leads, quotes | ❌ (a `SalesAgent` is reserved, not built) | **Primary gap, now scheduled and unblocked.** Records & pipeline exist and have deepened; nothing works them. Leads + the enrichment port shipped (design Phase A). Sequences and the agent are [PRD-010](./prd/PRD-010-sales-agent.md) / S15; **PRD-002's guardrails and eval runner have landed**, so S15 extends them rather than waiting on them ([docs](./modules/agents.md)). See [sales design](./architecture/sales-module-design.md). |
